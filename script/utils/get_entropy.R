...
#Function parameters
#.plt_model fitted PlackettLuce tree used for predictions
...
get_entropy <- function(.plt_model, .newdata, .pred_best, .nsim = 1000){
  ...
  res <- get_entropy_gen(avg_entr, .pred_best)
...
#Wu, Y.-C., Shih, M.-C., & Tu, Y.-K. (2021). Using Normalized Entropy to Measure Uncertainty of Rankings for Network Meta-analyses. Medical Decision Making, 41(6), 706-713. doi:10.1177/0272989x21999023
...
get_entropies <- function(rankProb){
  ...
  for(i in 1:length(rankProb)) entropies_list[[i]] <- entropy_arm_function(rankProb[[i]], ncol(rankProb[[i]]))
...
get_entropy_gen <- function(avg_entr_gen, pred_gen){
