...
dt <- read.csv("data/processed/red_beans_data.csv")
#load("data/agera5_temp/temp_rbca.rda")

load("data/raw/agera5/rbca_maxDT_agera5.rda")
load("data/raw/agera5/rbca_minNT_agera5.rda")

head(rbca_maxDT)
head(rbca_minNT)

#max(rbca_maxDT[[208]]) + 5
#max(rbca_maxDT[[1062]]) + 5
# dt$hdate <- climatrends:::.coerce2Date(dt$hdate)
# dt$pdate <- climatrends:::.coerce2Date(dt$pdate)

dt$hdate <- as.Date(dt$hdate)
dt$pdate <- as.Date(dt$pdate)

#keep <- format(dt$hdate, "%Y") < 2019
...
for(i in seq_len(nr)){
  tmax <- as.vector(rbca_maxDT[[i]])
  tmin <- as.vector(rbca_minNT[[i]])
...
# combine vectors
#dt <- cbind(dt, tveg = tveg, tflo = tflo, tgra = tgra, fdate = fdate, gdate = gdate, ind)

phenological_indices <- cbind(tveg = tveg, tflo = tflo, tgra = tgra, fdate = fdate, gdate = gdate, ind)
phenological_indices

write.csv(phenological_indices, "data/phenological_indices.csv", row.names = F)

#write.csv(dt, "data/red_beans_data_with_indices.csv", row.names = F)
